...
export default function ArtworkCard({ artwork, auction, onAddToCart }) {
  ...
  const wishlistIds = useMemo(() => {
    ...
    // store as IDs, but tolerate objects if they exist
    return raw
  ...
  const isWishlisted = useMemo(() => {
    return !!artwork?.id && wishlistIds.includes(artwork.id);
  ...
  const handleToggleWishlist = async (e) => {
    ...
    // If not logged in, redirect to login and carry the intent (wish=artworkId)
    if (!user?.uid) {
    ...
    try {
      ...
      // Refresh auth context so other UI updates (profile page, etc.)
