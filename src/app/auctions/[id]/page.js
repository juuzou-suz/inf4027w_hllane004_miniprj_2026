  ...
  const handlePlaceBid = async (e) => {
    ...
    try {
      ...
      if (amount <= auction.currentBid) {
        setError(`Bid must be higher than current bid of R${auction.currentBid}`);
        setBidding(false);
        return;
      }

      if (amount < auction.currentBid + auction.minimumIncrement) {
        setError(`Bid must be at least R${auction.currentBid + auction.minimumIncrement}`);
        setBidding(false);
        return;
      }
              ...
              {auction.status === 'live' && (
                <div>
                  ...
                  {user ? (
                    ...
                    <Link
                      href={`/login?redirect=${encodeURIComponent(`/auctions/${auctionId}`)}`}
